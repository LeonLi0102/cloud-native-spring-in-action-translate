...
Deployment 为我们提供了一个便捷的抽象，让我们可以声明想要达到的目标（期望状态），然后让 Kubernetes 去实现。您不需要担心如何实现特定的结果。与 Ansible 或 Puppet 等命令式工具不同，您只需告诉 Kubernetes您想要什么，编排器会找出如何实现期望结果并保持一致。这就是我们所说的声明式配置（declarative configuration）。
