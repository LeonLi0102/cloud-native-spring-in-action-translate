...
在本章中，我们使用了资源清单来在 Kubernetes 集群中创建 Deployment 和 Service。清单是"JSON 或 YAML 格式的 Kubernetes API 对象规范"。它指定了"Kubernetes 在应用清单时将维护的对象期望状态"（[https://kubernetes.io/docs/reference/glossary](https://kubernetes.io/docs/reference/glossary)）。
...
有几种方法可以针对 Kubernetes API 验证 Kubernetes 清单。我们将使用 Kubeval（[www.kubeval.com](http://www.kubeval.com)），一个开源工具。您可以在附录 A 的 A.4 节中找到安装说明。
