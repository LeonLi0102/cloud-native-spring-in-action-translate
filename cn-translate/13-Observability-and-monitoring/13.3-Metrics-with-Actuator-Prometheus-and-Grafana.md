...
结果是你在应用里探查指标集合，通过把某个指标的名称附加到端点上做进一步调查性，(例如，/actuator/metrics/jvm.memory.used)。
...
这种格式基于纯文本，称为 Prometheus exposition format。由于 Prometheus 被广泛用于生成和导出指标，这种格式已在 OpenMetrics（https://openapie.io，CNCF 孵化项目）中得以整合和标准化。根据 HTTP 请求的 Accept 头，Spring Boot 同时支持原始的 Prometheus 格式（默认行为和 OpenMetrics。如果你想取得 OpenMetrics 格式的指标，需要专门指定这个请求参数：
...
Spring Boot Actuator 依赖 Micrometer 插桩，并自动配置为为你的应用中用到的不同技术生成指标：JVM、日志器、Spring MVC、Spring WebFlux、RestTemplate、WebClient、数据源、Hibernate、Spring Data、RabbitMQ 等。
...
此前你已经与之前介绍日志时遇到过 Grafana。就像用 Loki 作为数据源浏览日志一样，你可以用 Prometheus 作为数据源查询指标。此外，你可以使用 Prometheus 存储的指标来定义仪表盘、可视化地呈现、再到指标返回已知的关键值时设置告警或通知。例如，当每分钟失败 HTTP 请求的比率超过一定阈值，你可能希望收到告警或通知来采取行动。图 13.5 展示了监控架构。

在 Polar Deployment 项目（polar-deployment）中，更新 Docker Compose 配置（docker-compose.yml）来包含 Prometheus。Grafana 已经在你可以开始之前导入到你项目中的配置文件中配置为使用 Prometheus 作为数据源，这些文件来自 Chapter13/13-end/polar-deployment/docker/observability。
...
services:
  ...
  grafana:
    ...
    depends_on:
      - loki             # 确保 Prometheus 在 Grafana 之前启动
      - prometheus
    ...
...
例如，打开 JVM 仪表盘（图 13.7）。它可视化与运行 Spring Boot 的 JVM 的不同相关的指标，如 CPU 使用率、堆内存、非堆内存、垃圾回收和线程。

在排除了 Dashboard 的其他仪表盘后，可以探索其他我配置的仪表盘，以便更好地了解 Polar Bookshop 应用。每个仪表盘都提供了目标和使用方式的附加信息。

在 Grafana 中查看应用指标完成后，停止所有容器（docker-compose down）。

### 13.3.3 在 Kubernetes 中配置 Prometheus 指标

在 Kubernetes 中 dài应用时，我们可以使用专用注解来标记 Prometheus 服务器应该抓取那些容器，并告知抓取 HTTP 端点和端口号。

稍后你会有机会在本章稍后测试该设置，我们会把完整的 Grafana 可观测性技术栈部署到生产 Kubernetes 集群。目前，先为 Polar Bookshop 中所有 Spring Boot 应用的 Deployment 清单做准备。比如下面的清单展示了如何修改可以进一步编辑 Catalog Service 清单（catalog-service/k8s/deployment.yml）。
