...
Spring Security（[https://spring.io/projects/spring-security](https://spring.io/projects/spring-security)）是保护 Spring 应用程序安全的事实标准，支持命令式和响应式技术栈。它提供身份验证和授权功能，以及针对最常见的攻击的保护。
...
然后运行 Edge Service 应用程序（`./gradlew bootRun`），打开浏览器窗口并访问 [http://localhost:9000/books](http://localhost:9000/books)。您应该会被重定向到 Spring Security 提供的登录页面，您可以在其中进行身份验证。
