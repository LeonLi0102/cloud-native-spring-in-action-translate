...
* **Spring Web**（`org.springframework.boot:spring-boot-starter-web`）提供了使用 Spring MVC 构建 Web 应用所需的库，并包含 Tomcat 作为默认的内嵌服务器。
...
Spring Boot 的一大亮点是它处理依赖管理的方式。像 `spring-boot-starter-web` 这样的启动器依赖让您无需再管理更多的依赖，也无需验证您导入的特定版本是否彼此兼容。这是 Spring Boot 的又一项功能，能让您以简单而高效的方式快速入门。

在下一节中，您将了解更多关于 Spring Boot 中内嵌服务器的工作原理以及如何配置它的内容。
