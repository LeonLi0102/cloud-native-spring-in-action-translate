...
Keycloak 启动并运行后，打开浏览器窗口，转到 [http://localhost:8080](http://localhost:8080)，使用 Docker Compose 文件中定义的相同凭据（user/password）登录管理控制台，并选择 PolarBookshop 领域。然后从左侧菜单中选择 Client Scopes。在新页面上（图 12.4），您将找到 Keycloak 中所有预配置范围的列表，并且可以选择创建新的范围。
