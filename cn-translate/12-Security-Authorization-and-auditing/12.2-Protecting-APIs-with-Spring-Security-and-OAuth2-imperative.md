...
![](../assets/final/ch12_12.6.png)
**图 12.6 Access Token (JWT) 中列出的用户角色如何转换为 Spring Security 用于 RBAC 的 GrantedAuthority 对象**

> **注意** 您可能想知道为什么我们使用 `SCOPE_` 或 `ROLE_` 前缀。由于授予权限可以用于表示不同的项目（角色、范围、权限），Spring Security 使用前缀对它们进行分组。我们将在 Polar Bookshop 示例中依赖此默认命名约定，但可以使用不同的前缀，甚至不使用前缀。有关更多信息，请参阅 Spring Security 文档（https://spring.io/projects/spring-security）。
