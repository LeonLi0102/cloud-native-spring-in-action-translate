...
Polar Bookshop 系统需要一个边缘服务器来将流量路由到内部 API 并处理多个跨切面关注点。你可以从 Spring Initializr（[https://start.spring.io](https://start.spring.io)）初始化新的 Edge Service 项目，将结果存储到新的 edgeservice Git 仓库中，并推送到 GitHub。初始化参数如图 9.2 所示。
